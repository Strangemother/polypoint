...
;const polypointEditor = (function(){
    ...
    const startView = function(conf=polypointEditorConfig) {
        ...
        installFirstValue(conf.editorConfig, initValue);
    }
    ...
    const installFirstValue = function(editorConfig, initValue=undefined) {

        let value = initValue == undefined? getStoredValue(): initValue
        ...
        let firstFile = editorConfig.firstFile || '/static/js/editor/init-example.js';
        fetch(firstFile)
            .then(
                (r)=>r.text()
            )
