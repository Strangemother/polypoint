...
class MainStage extends Stage {
    ...
    mounted(){
        ...
        this.rotationOriginOffset = new Point({x: 120, y: -40, z: 0})
    ...
    draw(ctx){
        ...
        let item = this.projectedCoin
        let z = item.p.z
        let red = deepColor - ((z / maxDepth) * deepColor)
        let color = `hsl(${red} 66% 35%)`

        // Keep the same explicit 2-segment cubic casting for the coin profile.
        let curve = this.getProjectedEggBezier(item, this.pointCurveConfig)
