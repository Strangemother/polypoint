...
class GearBox extends Stage {
    ...
    generate(pointCount=2){
        ...
        let ps = this.randomPoints = new PointList(
            ...
            new Point({x:700, y:200, radius: 20, angularVelocity: 0}),

            new Point({x:600, y:400, radius: 100, angularVelocity: 0}),
            new Point({x:600, y:100, radius: 20, angularVelocity: 0}),

            new Point({x:659, y:500, radius: 120, angularVelocity: 0}),
            new Point({x:150, y:450, radius: 20, angularVelocity: 0}),

            new Point({x:180, y:150, radius: 140, angularVelocity: 0}),
            new Point({x:180, y:150, radius: 15, angularVelocity: 0}),

            new Point({x:100, y:200, radius: 70, motor: 1, angularVelocity: 0}),
            new Point({x:800, y:300, radius: 70, motor: -.5, angularVelocity: 0})
        )

        this.bindPinionWheels(ps[0], ps[1])
        this.bindPinionWheels(ps[2], ps[3])
        this.bindPinionWheels(ps[4], ps[5])
        this.bindPinionWheels(ps[6], ps[7])
...
class MainStage extends GearBox {
