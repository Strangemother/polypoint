/*
title: Spaceship Vector Engines
categories: relative
files:
...
class MainStage extends MainStagePlainShip {
    ...
    impart(speed=1, direction=new Point(1,0)){
        ...
        let multSpeed = 1 - speed * 10
        // console.log('Impart speed', multSpeed)
        // for vector ship
        this.engines.forEach((engine, i) => {
            let force = ((engine.force + speed) * multSpeed) * .1
            // console.log('Impart speed', force)
            engine.force = force * -100
            // engine.label = force
        })
    }
    ...
    //     // this.addMotion(this.ship)
    //     applyEngineForces(this.ship, this.engines)

    //     // Apply motion!
    //     ship.x += ship.vx;
    //     ship.y += ship.vy;

    //     updateEnginePositions(this.ship, this.engines, this.engineOffsets)
    //     this.engines.forEach((engine, i) => {
    //         engine.force *= .9
    //     })
    // }
    ...
    renderVectorShip(ctx){
        ...
        this.engines.forEach(e=>{
            e.text.string(ctx, e.force.toFixed(5))
        })
