...
class MainStage extends Stage {
    ...
    drawLines(ctx){
        if(this.downPoint && this.upPoint){
            ...
            //             )
            let boxPoints = twoPointsAsRectOrdered(this.downPoint, this.upPoint)

            ;(new PointList(boxPoints[0], boxPoints[2])).cast().pen.fill(ctx, 'pink');

            // let mouseOver = this.mouse.point.within.polygon(boxPoints)
            // if(mouseOver) {
